...
" "$config_file")
  ...
  echo "$repos_json" | jq -c '.[]' | while IFS= read -r repo_config; do
    ...
    # Shallow clone, then drop .git entirely. The agent only needs a working
    # tree to read source — it never fetches or pushes. Removing .git keeps
    # the clone token (embedded in .git/config) out of the openshell sandbox,
    # where the agent processes untrusted reports, and shrinks the workdir
    # upload to just the current source.
    echo "Cloning $repo_name..."
    rm -rf "$clone_dir"
    git clone --depth 1 \
      "https://x-access-token:${GITHUB_TOKEN}@github.com/${repo_name}.git" \
